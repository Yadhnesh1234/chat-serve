...
const calculateTotalBill = (req, res) => {
  ...
  // Query to select orders for the given table with product details
  const sql = `
    SELECT 
      o.*, 
      p.product_name, 
  ...
  connection.query(sql, [table_id], (err, orders) => {
    if (err) {
      ...
      return res.status(500).json({ error: 'Error calculating total bill' });
    }

    // Calculate total bill
    const totalBill = orders.reduce((total, order) => total + order.total, 0);

    res.json({ total_bill: totalBill, orders: orders });
  });
};
